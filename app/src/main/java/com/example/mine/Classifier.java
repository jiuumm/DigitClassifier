...
public class Classifier {
    ...
    private ByteBuffer converBitmapToGrayByteBuffer(Bitmap bitmap){
    ...
    public Pair<Integer, Float> classify(Bitmap image){
        ByteBuffer buffer = converBitmapToGrayByteBuffer(resizeBitmap(image));

        ByteBuffer inputBuffer = buffer.duplicate();  // 복제
        inputBuffer.rewind();  // 위치 초기화
